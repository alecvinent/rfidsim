...
struct type_of_size {
  typedef typename conditional<N == sizeof(typename List::head),
          typename List::head,
...
private:
  ...
  typedef struct
  { unsigned char data[ sizeof(value_type) ];}aligned_storage_t;
  ...
  typedef struct
  { align_as_type data[ 1 + ( sizeof(value_type) - 1 ) / sizeof(align_as_type) ];}aligned_storage_t;
  aligned_storage_t data;
...
optional_noexcept          : has_value_( false )
          , contained()
          {}
...
#endif
          {
            ...
            { initialize( *rhs ); rhs.reset();}
            else if ( has_value() == true && rhs.has_value() == false )
            { rhs.initialize( **this ); reset();}
          }
...
template<typename T> bool operator==(optional<T> const & x,
                                     optional<T> const & y)
{
